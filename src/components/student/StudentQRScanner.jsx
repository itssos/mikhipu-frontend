...
export default function StudentQRScanner({ onScan }) {
    ...
    // Esta función SIEMPRE reinicia si keepScanningRef.current es true
        ...
        try {
            await qrCodeScanner.start(
                ...
                async (decodedText) => {
                    ...
                    toast.success("Escaneado: " + decodedText, { autoClose: 1200 });
                    ...
                    }, 1200); // Espera a que el toast termine o el usuario mueva el QR
                },
                () => {}
            );
        } catch (err) {
    ...
    return (
        <div className="max-w-md mx-auto p-4 rounded-2xl shadow-xl bg-white ">
            ...
            <div className="mb-4">
                <label className="block font-semibold mb-1">Select Camera:</label>
                <select
            ...
            <p className="text-xs mt-4 text-center text-gray-500 ">
                Works on PC and mobile. For better results on mobile, use the back camera.
            </p>
        </div>
    );
}
